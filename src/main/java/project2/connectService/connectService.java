...
public class connectService {
	...
	public String getMoves() throws Exception {
		String getUrl = url + "?type=moves&gameId=1171&count=1"; // test get
																	// members
																	// of
		...
		if (response.toString().contains("1070")) {
			...
			for (int i = index + 7; i < response.toString().length(); i++) {
				if (Character.isDigit(response.toString().charAt(i))) {
					res.append(response.toString().charAt(i));
				}else if (response.toString().charAt(i) == ',') {
					res.append(response.toString().charAt(i));
				}
			}
	...
	public void makeMove(String move) throws Exception {
		URL obj = new URL(url);
		HttpURLConnection conn = (HttpURLConnection) obj.openConnection();
		String parameters = "teamId=1051&type=move&gameId=1171&move=";
